...
| 6 | _pending deploy_ | ADR-0017 baseline: Engine A decomposed into 7 specific verbs (`mesh:lookupOwnership`/`mesh:traceLineage`/`mesh:assessImpact`/`mesh:findSchema`/`mesh:checkFreshness`/`mesh:filterByTag`/`mesh:describeAsset`) + generic fallback; per-verb prompts (reasoning patterns scoped to verbs that need them); agent echoes `output_uri` in final_answer; cortex-bff threads `routed_verb_iri` to `/analyze` and `output_uri` to `/render_ui`; Engine F advertises 9 presentation capabilities, deterministic hand-construction for KNOWLEDGE_DOCUMENT shapes, BAML with archetype-hint for the rest, legacy `DesignUI` only when `output_uri` is missing or unknown. Supersedes ADR-0012. | _pending_ | _pending_ | _pending_ | _pending_ | Expected: Q2 Engine F regression closes (KNOWLEDGE_DOCUMENT deterministically picked for `mesh:OwnershipFact`); Q9 archetype quirk closes (HAZARD_DECLARATION deterministically picked for `mesh:TagFilterResult`); Q7 lineage rendering more stable (PROCESS_TOPOLOGY hinted via persona string). Routing precision improves where specific verb synonyms match (Q1/Q3/Q9/Q12 should route off the generic fallback). |
