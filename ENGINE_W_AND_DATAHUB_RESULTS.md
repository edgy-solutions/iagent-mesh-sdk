...
| **DataHub query suite (Engine A → Engine D)** post-ADR-0017 | ✅ Architecture: 12/12 routing to specific verbs + 12/12 deterministic archetypes from predicate-graph capability table; Content: 8/12 user-visible right (2 timeouts, 2 Engine A content regressions) | 53 min | Post-ADR-0017 baseline (Run 8). Routing decomposed into 7 specific Engine A verbs; archetype lookup is now a two-step predicate match instead of a BAML LLM choice. Q9 catalog_pii now deterministically picks HAZARD_DECLARATION (was ASSET_STATE_METRIC across Runs 1-6). Q2 own_alice produces its best content of any run. Open follow-ups: (a) tune `mesh:lookupOwnership` for inverse lookups, (b) implement ADR-0016 to address recurring Q9 Mem0 pollution, (c) raise 900s supervisor timeout or bound recursive lineage depth. |
