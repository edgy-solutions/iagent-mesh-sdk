...
| **DataHub query suite (Engine A → Engine D)** post-ADR-0017 | ✅ Architecture: 12/12 routing to specific verbs + 12/12 deterministic archetypes from predicate-graph capability table; Content: 11/12 user-visible right after the Run 9 retest (Q1 wrong-owner remains as a Mem0-pollution variant) | 53 min full suite + ~18 min retest | Post-ADR-0017 baseline (Run 8) + focused retest (Run 9 after Mem0 flush + supervisor timeout bump). Routing decomposed into 7 specific Engine A verbs; archetype lookup is now a two-step predicate match instead of a BAML LLM choice. Q9 catalog_pii now deterministically picks HAZARD_DECLARATION (was ASSET_STATE_METRIC across Runs 1-6). **Run 9 finding:** the three Run-8 "regressions" (Q3/Q8 timeouts, Q9 wrong answer) all healed with a single Mem0 collection drop — they were the same ADR-0016 pollution case, not architectural. Open follow-ups: (a) implement ADR-0016 two-stream Mem0, (b) tune `mesh:lookupOwnership` for inverse lookups (Q1 own_ds still wrong even after flush). |
