"""The SOURCE_LEDGER row vocabulary — ADR-0046's contract, not one host's convention.

MOVED HERE FROM `agent_fleet/graph_host/rows.py` FOR 0.9.3, and the argument is the one that
moved `enforce_refusal`: a convention only one host implements is one that route C — a team
running their own host — will not inherit. The ledger row is the contract every graph answers in,
so it belongs with the interfaces that promise it.

AND THE PRODUCER AND CONSUMER WERE IN DIFFERENT REPOS, which is the stronger reason and was
checkable rather than argued. `holes_from` below is the ONLY place `holes` is built;
`enforce_refusal` in `graph_manifest.py` READS `out.get("holes")` and refuses on it. Producer in
invincible-agent, consumer in the SDK — the same cross-repo split the edge-type registry needed a
conformance seal to survive. Here it is simply removed: one emission, one truth, one home.

It needed no changes to move: stdlib imports only (`__future__`, `typing`), no relative imports,
verified by AST rather than taken on description.
